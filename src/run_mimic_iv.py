...
from util import ddi_rate_score, Metrics, get_ehr_adj, multi_label_metric
...
parser.add_argument('--ddi', action='store_true', default=True, help='use ddi')
...
parser.add_argument('--use_mol_net', action='store_true', default=True, help='use the molecule encoding sub-network')
parser.add_argument('--mol_net_type', type=int, default=3, help='1--full, 2--atom only, 3--frag only')

parser.add_argument('--use_mol_loss', action='store_true', default=False, help='use the mol_loss (--use_mol_net must be true)')


args = parser.parse_args()
print(args)

if not os.path.exists(os.path.join("./src/saved_mimic4", args.model_name)):
		os.makedirs(os.path.join("./src/saved_mimic4", args.model_name))
...
def eval(model: main_model, data_eval, voc_size, epoch, metric_obj: Metrics):
	...
	for data_tensors in model.get_batch(data_eval, 16):#128):
...
def main():
	...
	args.resume_path = f'./src/saved_mimic4/{args.model_name}/best.model'
	...
	for epoch in range(epoch_begin, EPOCH):
		...
		#'''
		savefile = os.path.join('./src/saved_mimic4', args.model_name, 'Epoch_%d_JA_%.4f_DDI_%.4f.model' % (epoch, 0, 0))
		...
		if best_ja < ja:
			best_epoch = epoch
			best_ja = ja
			savefile = os.path.join('./src/saved_mimic4', args.model_name, 'best.model')
			torch.save({"model": model.state_dict(),
						"optimizer": optimizer.state_dict(),
						"epoch": epoch}, open(savefile, 'wb'))
	...
	dill.dump(history, open(os.path.join('./src/saved_mimic4', args.model_name, 'history_{}.pkl'.format(args.model_name)), 'wb'))
