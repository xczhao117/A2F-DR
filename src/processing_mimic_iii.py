...
def create_str_token_mapping(df):
    ...
    dill.dump(obj={'diag_voc':diag_voc, 'med_voc':med_voc ,'pro_voc':pro_voc}, file=open('./data/processed/voc_final.pkl','wb'))
...
def create_patient_record(df, diag_voc, med_voc, pro_voc):
    ...
    for subject_id in df['SUBJECT_ID'].unique():
        ...
        for index, row in item_df.iterrows():
            ...
            for one_atc3 in row['NDC']:
                ...
                mols = dill.load(open(f"./data/processed/{one_atc3}.pkl", 'rb'))
                for moldata in mols:
                    # print(moldata)
    ...
    dill.dump(obj=records, file=open('./data/processed/records_final.pkl', 'wb'))
    return records
...
def get_ddi_matrix(records, med_voc, ddi_file):
    ...
    dill.dump(ddi_adj, open('./data/processed/ddi_A_final.pkl', 'wb'))
...
if __name__ == '__main__':
    ...
    ddi_rate = ddi_rate_score(records, f"./data/processed/ddi_A_final.pkl")
